...
(async()=>{
  ...
  const log={};
  // two list items, alternate bullet ·
  await page.keyboard.press('Enter');
  await typeInline(page,'· 说思念，不直说想你，说**"海上生明月，天涯共此时"**');
  await page.keyboard.press('Enter');
  await typeInline(page,'· 说豁达，不写无所谓，写**"竹杖芒鞋轻胜马，谁怕？一蓑烟雨任平生"**');
  log.after=await page.evaluate(()=>document.querySelector('.ProseMirror').innerHTML.slice(0,400));
  fs.writeFileSync('E:/skills/toutiao-publish/content/_dbgd.log',JSON.stringify(log,null,2)+'\n');
