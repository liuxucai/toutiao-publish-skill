...
(async()=>{
  ...
  const log={};
  // simulate list item with • prefix + Chinese quotes at end
  await page.keyboard.press('Enter');
  await typeInline(page,'• 说思念，不直说想你，说**"海上生明月，天涯共此时"**');
  log.after=await page.evaluate(()=>document.querySelector('.ProseMirror').innerHTML.slice(0,300));
  fs.writeFileSync('E:/skills/toutiao-publish/content/_dbgc.log',JSON.stringify(log,null,2)+'\n');
